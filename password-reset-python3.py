...
import json, urllib.request, urllib.error, urllib.parse, time, string, random, subprocess, os, platform, sys, configparser
...
def save_reg():
    ...
    with open(timestamp_dir + "\\" + 'password-reset.reg','w') as configfile:
...
def reset_password_linux(password_length):
    password = id_generator(password_length)
    write_timestamp()
    print("INFO: Resetting passwor d to: %s" % password)
    cmd = ['/usr/bin/passwd', 'root']
    p = subprocess.Popen(cmd, stdin=subprocess.PIPE)
    ip_password = '%(p)s\n%(p)s\n' % {'p': password}
    p.stdin.write(str.encode(ip_password))
    p.stdin.flush()
    for x in range(0, 10):
